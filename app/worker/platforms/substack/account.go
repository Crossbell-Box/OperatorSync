...
func Account(username string, validateString string) (bool, uint, string, bool) {
	...
	collectLink := commonConsts.SUPPORTED_PLATFORM["substack"].FeedLink

	if rawFeed, errCode, err := utils.RSSFeedRequest(
		strings.ReplaceAll(collectLink, "{{username}}", username),
		true,
	); err != nil {
		global.Logger.Error("Failed to collect substack feeds of user ", username, " for account validate with error: ", err.Error())
