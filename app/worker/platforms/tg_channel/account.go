...
func Account(username string, validateString string) (bool, uint, string, bool) {
	...
	collectLink := commonConsts.SUPPORTED_PLATFORM["tg_channel"].FeedLink
	...
	if rawFeed, errCode, err := utils.RSSFeedRequestJson(
		...
		global.Logger.Error("Failed to collect tg_channel feeds of user ", username, " for account validate with error: ", err.Error())
