...
func Account(username string, validateString string) (bool, uint, string, bool) {
	...
	collectLink := commonConsts.SUPPORTED_PLATFORM["jike"].FeedLink
	...
	if rawFeed, errCode, err := utils.RSSFeedRequest(
		...
		global.Logger.Error("Failed to collect jike feeds of user ", username, " for account validate with error: ", err.Error())
